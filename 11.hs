...
readInput1 :: String -> _
readInput1 inp = M.fromList [((i, j), digitToInt c) | (i, l) <- zip [0..] $ lines inp, (j, c) <- zip [0..] l]
...
solve1 = run 0 100
  where
    run !flashed 0 _ = flashed
    run !flashed n !m = let (k, m') = sim m in run (flashed + k) (n - 1) m'
    sim m = go S.empty (M.keys m) m
      where
        go !flashed (p:ps) !m
          | m M.! p == 9
          = go (S.insert p flashed) (neighbors p m ++ ps) (M.adjust succ p m)
          | otherwise
          = go flashed ps (M.adjust succ p m)
        go !flashed [] m = (S.size flashed, M.map (\x -> if x > 9 then 0 else x) m)
    neighbors (i, j) m = filter (`M.member` m) $ filter (/= (i, j)) $ (,) <$> [i-1..i+1] <*> [j-1..j+1]

solve2 :: _ -> _
solve2 = run 0
  where
    run !n m = let (b, m') = sim m in
      if b then (n + 1)
      else run (n + 1) m'
    sim m = go S.empty (M.keys m) m
      where
        go !flashed (p:ps) !m
          ...
          | otherwise
          = go flashed ps (M.adjust succ p m)
        go !flashed [] m = (S.size flashed == M.size m, M.map (\x -> if x > 9 then 0 else x) m)
    neighbors (i, j) m = filter (`M.member` m) $ filter (/= (i, j)) $ (,) <$> [i-1..i+1] <*> [j-1..j+1]
