...
readInput1 :: String -> (Graph, Vertex -> (String, [String]), String -> Maybe Vertex)
readInput1 xs = (g, \v -> case t v of (_, k, ks) -> (k, ks), f)
  where (g, t, f) = graphFromEdges $ map (\xs -> ((), fst $ head xs, snd <$> xs)) $ groupBy ((==) `on` fst) $ sortOn fst $ concatMap (\p -> [p, swap p]) $ map ((\[a, b] -> (a, b)) . splitOn "-") $ lines xs
...
solve1 :: _ -> _
solve1 (g, t, f) = go S.empty startV
  where
    Just startV = f "start"
    Just endV = f "end"
    isBig v = case t v of (n, _) -> isUpper $ head n
    go !seen !v
      | v `S.member` seen = 0
      | v == endV = 1
      | isBig v = sum $ go seen <$> g ! v
      | otherwise = sum $ go (S.insert v seen) <$> g ! v

solve2 :: _ -> _
solve2 (g, t, f) = sum ((\twice -> go (Just twice) S.empty startV) <$> smalls) - (length smalls - 1) * solve1 (g, t, f)
  where
    smalls = filter (`notElem` [startV, endV]) $ filter (not . isBig) $ range $ bounds g
    Just startV = f "start"
    Just endV = f "end"
    isBig v = case t v of (n, _) -> isUpper $ head n
    go !twice !seen !v
      | v `S.member` seen = 0
      | v == endV = 1
      | isBig v = sum $ go twice seen <$> g ! v
      | Just v' <- twice, v == v' = sum $ go Nothing seen <$> g ! v
      | otherwise = sum $ go twice (S.insert v seen) <$> g ! v
