...
readInput1 :: String -> _
readInput1 = map ((\[a, b] -> (a, read @Int b)) . words) . lines
...
solve1 = go 0 0
  where
    go x y [] = x * y
    go x y (("down", d):xs) = go x (y + d) xs
    go x y (("up", d):xs) = go x (y - d) xs
    go x y (("forward", d):xs) = go (x + d) y xs

solve2 :: _ -> _
solve2 = go 0 0 0
  where
    go x y a [] = x * y
    go x y a (("down", d):xs) = go x y (a + d) xs
    go x y a (("up", d):xs) = go x y (a - d) xs
    go x y a (("forward", d):xs) = go (x + d) (y + d * a) a xs
