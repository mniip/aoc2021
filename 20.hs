...
readInput1 :: _ -> _
readInput1 = ((\xs -> listArray (0, length xs - 1) $ map (equals '#') xs) *** (\xs -> let xss = lines xs in listArray ((1, 1), (length xss, length $ head xss)) $ map (equals '#') $ concat $ transpose xss)) . splitOn2 "\n\n"
...
enhance :: Array Int Bool -> (Bool, Array (Int, Int) Bool) -> (Bool, Array (Int, Int) Bool)
enhance alg (def, arr) = (alg ! if def then 0x1FF else 0, array ((x1 - 1, y1 - 1), (x2 + 1, y2 + 1))
  [((x, y), alg ! foldl' (\n d -> n * 2 + fromEnum d) 0 [ind (x + dx, y + dy) | dy <- [-1..1], dx <- [-1..1]]) | y <- [y1 - 1 .. y2 + 1], x <- [x1 - 1 .. x2 + 1]])
  where
    ind p = if inRange (bounds arr) p then arr ! p else def
    ((x1, y1), (x2, y2)) = bounds arr

solve1 :: _ -> _
solve1 (alg, arr) = sum $ fmap fromEnum $ snd $ iterate (enhance alg) (False, arr) !! 2

solve2 :: _ -> _
solve2 (alg, arr) = sum $ fmap fromEnum $ snd $ iterate (enhance alg) (False, arr) !! 50
