...
solve1 :: [String] -> Int
solve1 xs = fromBin mc * fromBin (map inv mc)
  where
    inv '0' = '1'
    inv '1' = '0'
    mc = map leastCommon $ transpose xs
fromBin :: String -> Int
fromBin = foldl' (\n d -> n * 2 + if d == '1' then 1 else 0) 0
leastCommon :: String -> Char
leastCommon = fst . head . sortOn (snd &&& fst) . map (head &&& length) . group . sort

solve2 :: [String] -> Int
solve2 xs = go xs xs 0
  where
    go :: [String] -> [String] -> Int -> Int
    go oxy co2 i
      | i >= length (head oxy) = fromBin (head oxy) * fromBin (head co2)
      | otherwise = go (filter ((/= lcoxy) . (!! i)) oxy) (filter ((== lcco2) . (!! i)) co2) (i + 1)
      where
        lcoxy = leastCommon ((!! i) <$> oxy)
        lcco2 = leastCommon ((!! i) <$> co2)
