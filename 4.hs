...
readInput1 :: String -> ([Int], [Array (Int, Int) Int])
readInput1 = (\(xs:xss) -> (map read $ splitOn "," xs, (listArray ((0, 0), (4, 4)) . map read . words) <$> xss)) . splitOn "\n\n"
...
solve1 :: _ -> _
solve1 (xs, boards) = go S.empty xs
  where
    go s (x:xs)
      | Just b <- find (winning $ S.insert x s) boards = x * sum [n | n <- toList b, n `S.notMember` S.insert x s]
      | otherwise = go (S.insert x s) xs
    winning s b
      = any (\i -> all (\j -> b ! (i, j) `S.member` s) [0..4]) [0..4]
      || any (\j -> all (\i -> b ! (i, j) `S.member` s) [0..4]) [0..4]

solve2 :: _ -> _
solve2 (xs, boards) = go S.empty xs boards
  where
    go s (x:xs) bs = case filter (not . winning (S.insert x s)) bs of
      [] -> x * sum [n | n <- toList $ head bs, n `S.notMember` S.insert x s]
      bs -> go (S.insert x s) xs bs
    winning s b
      = any (\i -> all (\j -> b ! (i, j) `S.member` s) [0..4]) [0..4]
      || any (\j -> all (\i -> b ! (i, j) `S.member` s) [0..4]) [0..4]
