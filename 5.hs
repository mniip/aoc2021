...
readInput1 = map ((\[a, _, b] -> (point a, point b)) . words) . lines
  where point xs = case splitOn "," xs of [x, y] -> (read x, read y)
...
solve1 :: _ -> _
solve1 = finish . foldl' render M.empty
  where
    finish m = length $ filter (>= 2) $ M.elems m
    render m ((x1, y1), (x2, y2))
      | x1 == x2 = foldl' (\m y -> M.insertWith (+) (x1, y) 1 m) m [min y1 y2..max y1 y2]
      | y1 == y2 = foldl' (\m x -> M.insertWith (+) (x, y1) 1 m) m [min x1 x2..max x1 x2]
      | otherwise = m

solve2 :: _ -> _
solve2 = finish . foldl' render M.empty
  where
    finish m = length $ filter (>= 2) $ M.elems m
    render m ((x1, y1), (x2, y2)) = foldl' (\m p -> M.insertWith (+) (x1 + dx * p, y1 + dy * p) 1 m) m [0..d]
      where
