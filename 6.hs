...
readInput1 :: String -> _
readInput1 = map (read @Int) . splitOn ","
...
solve1 :: _ -> _
solve1 xs = length $ foldl' (const . sim) xs [1..80]
  where
    sim xs = map (\i -> if i == 0 then 6 else i - 1) xs ++ replicate (length $ filter (== 0) xs) 8

solve2 :: _ -> _
solve2 xs = sum $ M.elems $ foldl' (const . sim) (M.fromListWith (+) $ (,1) <$> xs) [1..256]
  where
    sim m = M.unionWith (+) (M.mapKeysMonotonic pred (M.delete 0 m)) (M.fromList [(6, k), (8, k)])
      where k = M.findWithDefault 0 0 m
