...
readInput1 :: String -> _
readInput1 xs = listArray ((1, 1), (length $ head xss, length xss)) $ concat $ transpose xss
  where xss = map (map digitToInt) $ lines xs
...
solve1 :: _ -> _
solve1 a = sum [1 + a ! (i, j) | (i, j) <- range $ bounds a
    , i == 1 || a ! (i, j) < a ! (i - 1, j)
    , i == w || a ! (i, j) < a ! (i + 1, j)
    , j == 1 || a ! (i, j) < a ! (i, j - 1)
    , j == h || a ! (i, j) < a ! (i, j + 1)
    ]
  where (_, (w, h)) = bounds a

solve2 :: _ -> _
solve2 a = product . take 3 . sortOn Down . map S.size $ start (S.fromList [p | p <- range $ bounds a, a ! p < 9])
  where
    start ind
      | Just m <- S.lookupMin ind = go ind S.empty [m]
      | otherwise = []
    go ind cur [] = cur : start ind
    go ind cur (w@(i, j):ws)
      | w `S.notMember` ind = go ind cur ws
      | otherwise = go (S.delete w ind) (S.insert w cur) $ (i - 1, j):(i + 1, j):(i, j - 1):(i, j + 1):ws
